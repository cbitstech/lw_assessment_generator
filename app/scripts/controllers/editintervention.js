...
angular.module('assessmentGeneratorApp')
  .controller('EditinterventionCtrl', function ($scope,$routeParams) {
    
  	$scope.currentInterventions = JSON.parse(localStorage.interventions);

  	$scope.selectedIntervention = _.where($scope.currentInterventions,{guid:$routeParams.guid})[0];
  	console.log($scope.selectedIntervention);
